...
class record_fetcher(object):
    ...
    methods:
      - fetch(batch_size,binary,seed)
        ...
        outputs:
          - numpy array representing training summary statistics and demographic data over each time window
          - numpy array representing testing summary statistics and demographic data over each time window
    ...
    def fetch(self,batch_size,binary=True,seed=None):
        ...
        #shuffle arrays
        comb = np.concatenate((X_train,y_train),1)
        np.random.shuffle(comb)
        X_train = comb[:,:-1]
        y_train = comb[:,-1]
        
        comb = np.concatenate((X_test,y_test),1)
        np.random.shuffle(comb)
        X_test = comb[:,:-1]
        y_test = comb[:,-1]
