...
class conv_lstm(object):
      ...
      - train(X_train, y_train)
        ...
        parameters:
          - X_train: 4d numpy array
            training features output by record_fetcher_within_subject_raw.py
          - y_train: 2d numpy array
            training labels output by record_fetcher_within_subject_raw.py
        outputs:
      ...
      - predict(X_test)
        ...
        parameters:
          - X_test: 4d numpy array
            testing features output by record_fetcher_within_subject_raw.py
        outputs:
    ...
    def __init__(self,binary=False):
        ...
        #layer 5 - lstm units        
        self.Wi = theano.shared(self.ortho_weight(64),borrow=True)
        self.Wf = theano.shared(self.ortho_weight(64),borrow=True)
        self.Wc = theano.shared(self.ortho_weight(64),borrow=True)
        self.Wo = theano.shared(self.ortho_weight(64),borrow=True)
        self.Ui = theano.shared(self.ortho_weight(64),borrow=True)
        self.Uf = theano.shared(self.ortho_weight(64),borrow=True)
        self.Uc = theano.shared(self.ortho_weight(64),borrow=True)
        self.Uo = theano.shared(self.ortho_weight(64),borrow=True)
        self.bi = theano.shared(np.zeros((64,), dtype=theano.config.floatX),borrow=True)
        self.bf = theano.shared(np.zeros((64,), dtype=theano.config.floatX),borrow=True)
        self.bc = theano.shared(np.zeros((64,), dtype=theano.config.floatX),borrow=True)
        self.bo = theano.shared(np.zeros((64,), dtype=theano.config.floatX),borrow=True)
        self.C0 = theano.shared(np.zeros((64,), dtype=theano.config.floatX),borrow=True)
        self.h0 = theano.shared(np.zeros((64,), dtype=theano.config.floatX),borrow=True)

        #layer 6 - lstm units        
        self.Wi2 = theano.shared(self.ortho_weight(64),borrow=True)
        self.Wf2 = theano.shared(self.ortho_weight(64),borrow=True)
        self.Wc2 = theano.shared(self.ortho_weight(64),borrow=True)
        self.Wo2 = theano.shared(self.ortho_weight(64),borrow=True)
        self.Ui2 = theano.shared(self.ortho_weight(64),borrow=True)
        self.Uf2 = theano.shared(self.ortho_weight(64),borrow=True)
        self.Uc2 = theano.shared(self.ortho_weight(64),borrow=True)
        self.Uo2 = theano.shared(self.ortho_weight(64),borrow=True)
        self.bi2 = theano.shared(np.zeros((64,), dtype=theano.config.floatX),borrow=True)
        self.bf2 = theano.shared(np.zeros((64,), dtype=theano.config.floatX),borrow=True)
        self.bc2 = theano.shared(np.zeros((64,), dtype=theano.config.floatX),borrow=True)
        self.bo2 = theano.shared(np.zeros((64,), dtype=theano.config.floatX),borrow=True)
        self.C02 = theano.shared(np.zeros((64,), dtype=theano.config.floatX),borrow=True)
        self.h02 = theano.shared(np.zeros((64,), dtype=theano.config.floatX),borrow=True)
        
        #layer 7 - softmax
        if binary:
            self.W2 = theano.shared(self.ortho_weight(64)[:,:2],borrow=True)
            self.b2 = theano.shared(np.zeros((2,), dtype=theano.config.floatX),borrow=True)
        else:
            self.W2 = theano.shared(self.ortho_weight(64)[:,:5],borrow=True)
        ...
        self.fullparams = self.params1 + self.params2 + [self.conv_w1,self.conv_b1,self.conv_w2,self.conv_b2,self.conv_w3,self.conv_b3,self.conv_w4,self.conv_b4,self.h0,self.C0,self.h02,self.C02,self.W2,self.b2]
...
if (len(sys.argv) < 2):
    print 'Usage: python convlstm_within_subject.py <1 for 2-category labels, 0 for 5-category labels>'
    exit(1)
