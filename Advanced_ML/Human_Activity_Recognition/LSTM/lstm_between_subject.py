...
class lstm(object):
      ...
      - train(X_train, y_train)
        ...
        parameters:
          - X_train: 2d numpy array
            training features output by record_fetcher_between_subject.py
          - y_train: 2d numpy array
            training labels output by record_fetcher_between_subject.py
        outputs:
      ...
      - predict(X_test)
        ...
        parameters:
          - X_test: 2d numpy array
            testing features output by record_fetcher_between_subject.py
        outputs:
    ...
    def __init__(self,binary=False):
        ...
        #layer 1 - lstm units
        self.input = T.matrix()
        self.Wi = theano.shared(self.ortho_weight(109)[:,:32],borrow=True)
        self.Wf = theano.shared(self.ortho_weight(109)[:,:32],borrow=True)
        self.Wc = theano.shared(self.ortho_weight(109)[:,:32],borrow=True)
        self.Wo = theano.shared(self.ortho_weight(109)[:,:32],borrow=True)
        ...
        #final softmax, final output is the output at the final timestep
        self.output = T.nnet.softmax(T.dot(self.h_output1,self.W2)+self.b2)[-1:,:]
        ...
        self.updates = self.adam(self.cost,self.params1+[self.h0,self.C0,self.W2,self.b2])
